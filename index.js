...
// Define a simple route to respond with a status message
app.get('/', (req, res) => {
...
// File for storing speedruns
const speedrunsFile = "speedruns.json";

// Load saved speedruns
let speedruns = {};
...
const saveSpeedrun = (game, time, user) => {
    ...
    // Update placements
    speedruns[game] = speedruns[game].map((run, index) => ({
        ...run,
        placement: index + 1,
    }));
    ...
    try {
        fs.writeFileSync(
            speedrunsFile,
            JSON.stringify(speedruns, null, 4),
            "utf8",
        );
    } catch (err) {
...
const removeSpeedrun = (game, placement, user) => {
    ...
    const indexToRemove = speedruns[game].findIndex(
        (run) => run.user === user && run.placement === parseInt(placement),
    );
...
const showUserSpeedruns = (user) => {
    ...
    for (const [game, runs] of Object.entries(speedruns)) {
        ...
        if (userRuns.length > 0) {
            const runsText = userRuns
                .map(
                    (run) =>
                        `${game} - Platzierung: ${run.placement}, Zeit: ${run.time}`,
                )
                .join("\n");
            userSpeedruns.push(runsText);
        }
    }
    return userSpeedruns.length > 0
        ? userSpeedruns.join("\n")
...
client.on("messageCreate", async (message) => {
    ...
    if (content.startsWith("!speedrun")) {
        ...
        if (lastSpaceIndex === -1) {
            message.channel.send(
                'Bitte gib das Spiel und die Zeit im Format "HH:MM:SS.MS" an. Beispiel: !speedrun Super Mario 64 01:23.45',
            );
            return;
        }

        const gameTitle = capitalizeFirstLetter(
            args.slice(0, lastSpaceIndex).trim(),
        );
        const timeFormat = args.slice(lastSpaceIndex + 1).trim();

        if (!isValidTimeFormat(timeFormat)) {
            message.channel.send(
                'Die Zeit ist im falschen Format. Bitte benutze "HH:MM:SS.MS". Beispiel: !speedrun Super Mario 64 01:23.45',
            );
            return;
        }
    ...
    if (content.startsWith("!remove")) {
        ...
        const gameTitle = capitalizeFirstLetter(
            gameTitleParts.join(",").trim(),
        );
        const placement = placementStr.trim();

        if (!placement || !gameTitle) {
            message.channel.send(
                'Bitte gib den Speedrun im Format "Platzierung,Spiel" an, um den Speedrun zu entfernen. Beispiel: !remove 10,Super Mario 64',
            );
            return;
        }
        ...
        message.channel.send(
            success
                ? "Speedrun entfernt!"
                : "Der angegebene Speedrun existiert nicht.",
        );
    }
    ...
    if (message.content.startsWith("!user")) {
        ...
        if (!input) {
            message.channel.send(
                "Bitte gib den Benutzernamen oder Nicknamen an, für den du die Speedruns sehen möchtest.",
            );
            return;
        }
        ...
        if (!member) {
            message.channel.send(
                "Benutzer nicht gefunden. Bitte stelle sicher, dass der Name korrekt ist.",
            );
            return;
        }
        ...
        try {
            ...
            if (userSpeedruns) {
                message.channel.send(
                    `Speedruns von **${userTag}**:\n${userSpeedruns}`,
                );
            } else {
                message.channel.send(
                    "Keine Speedruns für diesen Benutzer gefunden.",
                );
            }
        } catch (error) {
            console.error("Fehler beim Abrufen der Speedruns:", error);
            message.channel.send(
                "Beim Abrufen der Speedruns ist ein Fehler aufgetreten.",
            );
        }
    }
    ...
    if (content.startsWith("!liste")) {
        ...
        if (!args) {
            message.channel.send(
                "Bitte gib das Spiel an, für das du die Liste sehen möchtest.",
            );
            return;
        }
        ...
        speedruns[game].forEach((run) => {
            if (
                !bestSpeedruns.has(run.user) ||
                bestSpeedruns.get(run.user).time > run.time
            ) {
                bestSpeedruns.set(run.user, run);
            }
        });

        const sortedSpeedruns = Array.from(bestSpeedruns.values()).sort(
            (a, b) => {
                const timeToSeconds = (time) => {
                    const [hours, minutes, seconds, milliseconds] = time
                        .split(/[:.]/)
...
async function sendOfflineMessage() {
    ...
    if (channel) {
        await channel.send(
            "Der Bot ist gerade offline, weil Kai Strom sparen will! Er war online für " +
                getElapsedTime() +
                ".",
        );
    }
...
client.once("ready", () => {
    ...
    client.channels
        ...
        .then((channel) => {
            if (channel) {
                channel.send(
                    "Der Bot ist Jetzt Online, weil Kai ausnahmsweise keinen Strom sparen will!",
                );
            }
        })
        .catch((error) => {
